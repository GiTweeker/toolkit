	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			...
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				//allowedTypes := []string{"image/gif", "image/jpeg", "image/png"}

				if len(t.AllowFileTypes) > 0 {
					for _, x := range t.AllowFileTypes {
...
func (t *Tools) Sluggify(s string) (string, error) {
	...
	if len(slug) == 0 {
		return "", errors.New("after removing characters, slug is empty")
	}
